import XCTest
@testable import CuttingBoardCare

final class CuttingBoardCareTests: XCTestCase {
    ...
    func test_addEntry_increasesCount() {
        ...
        let entry = Entry(boardName: "Test", date: Date(), note: "note")
    ...
    func test_addEntry_blockedAtFreeLimit() {
        ...
        while store.entries.count < Store.freeLimit {
            _ = store.add(Entry(boardName: "Filler", date: Date(), note: ""), isPro: false)
        }
        let result = store.add(Entry(boardName: "Overflow", date: Date(), note: ""), isPro: false)
    ...
    func test_addEntry_proBypassesLimit() {
        ...
        let result = store.add(Entry(boardName: "ProEntry", date: Date(), note: ""), isPro: true)
    ...
    func test_deleteEntry_removesFromList() {
        let store = Store()
        let entry = Entry(boardName: "ToDelete", date: Date(), note: "")
        _ = store.add(entry, isPro: false)
        store.delete(entry)
    ...
    func test_updateEntry_changesFields() {
        let store = Store()
        var entry = Entry(boardName: "Original", date: Date(), note: "")
        _ = store.add(entry, isPro: false)
        entry.boardName = "Updated"
        store.update(entry)
        XCTAssertEqual(store.entries.first(where: { $0.id == entry.id })?.boardName, "Updated")
    }
